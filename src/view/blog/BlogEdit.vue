  ...
  <Window
    ...
    <Dialog
      ...
      <el-form
        ...
        <el-form-item label="原文地址" prop="type" v-if="blogFormData.type==1">
            <el-input
            v-model="blogFormData.reprintUrl" placeholder="请输入原文地址"></el-input>
        </el-form-item>
        ...
        <el-form-item label="博客标签" prop="categoryDesc">
          <div class="tag-input-panel">
            <div class="tag-list">
              <el-tag v-for="(item,index) in blogFormData.tag" @close="closeTag(index)" :key="item" closable>{{ item }}</el-tag>
            </div>
...
const autoSave=async ()=>{
  if(blogFormData.value.title=="" && blogFormData.value.content=="" ||startTimer==null){
    return
  }
  const params={}
